...
function renderVideos() {
  ...
  if (!videos.length) {
    grid.innerHTML = `
      <div class="video-placeholder">
        <strong>New episodes are on the way 🎙️</strong>
        <p>Add your YouTube links in <code>js/videos.js</code> and they'll show up here automatically.</p>
      </div>`;
    return;
  }
  ...
  grid.innerHTML = videos.map(v => {
    ...
    const embed = id
      ? `<iframe src="https://www.youtube.com/embed/${id}" title="${escapeHtml(v.title || 'Episode')}"
    ...
    return `
      <div class="video-card">
        ...
        <div class="video-info">
          <h3>${escapeHtml(v.title || 'Untitled episode')}</h3>
